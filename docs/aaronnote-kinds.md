...
- `surface`: the rendered `.cm-editor` / published `#content` element
...
Do not assume the rendered note DOM is stable at the exact moment `setup()` first runs. Aaronnote may still be replacing the cm-editor DOM, and published pages may still be finishing module execution. A kind that scans headings only once can silently see zero or one slide, then leave the page looking like normal Markdown.
...
title-align: left
css:
  body[data-note-kind="slides"] .cm-editor h1 { color: #b42318; }
#+end slideGConfig
```
