...
本地插件放在 `plugin/<id>/`，由 server 的 `/api/plugins` 扫描 `plugin.json`，再由 `Aaronnote/aaronnote/main.ts` 通过 Vite `import.meta.glob("../../plugin/*/*.ts")` 加载入口。autoload 插件随编辑器启动；非 autoload 插件可以通过插件管理页 action 临时启动。

`plugin/roamlookup` 是交互式知识库查询插件。它把 `Roam lookup` 挂到 Notes 页 tab 下，前端只负责会话 UI、用户 idle timer 和关闭按钮；真正的 Codex 调用在 server 侧 `/api/roamlookup/*`。server 继承当前进程环境启动 `codex exec`，因此使用同一套 Codex 登录 token，并且固定 `--sandbox read-only`。会话一分钟无交互或收到 close 请求时会关闭 session，正在运行的子进程也会被终止。
...
插件、snippet、补全、quick insert 这类“在当前光标处输入文字”的功能，优先使用 `editor.insertText(text, deleteBefore?)`。这条路径会落到当前活动 surface：渲染模式下是 ProseMirror transaction，source 模式下是 textarea range edit，因此能保留 selection、history、normalize 和 NodeView 状态。

不要把普通输入/补全实现成 `replaceMarkdownRange()`。`replaceMarkdownRange()` 接收 markdown-source offset，在渲染模式下会重建整篇 Markdown，适合有明确 source 坐标且确实要重写 Markdown 的命令，不适合 Copilot/snippet 这种光标处插入。补全服务如果返回“替换当前 prefix”的 range，应先裁掉已经存在的 prefix，再把剩余文本交给 `editor.insertText()`。
...
`Aaronnote/src/command-syntax.ts` 放统一的自建命令语法解析：

- inline command: `@@cmd(switch) [context]{arg: value}`
- block command: `#+begin kind title ... #+end kind`

约定上只保留一种可读写法：inline command 的 `[` 前必须有空格；`{}` 里的参数使用 `key: value`。例如 TODO 使用：

```md
@@todo(doing) [write parser docs]{ddl: 2026-05-20}
```

`@@todo [plain task]` 没有 switch 时默认是 `todo`。Agenda 读取同一套 inline command 语义，`ddl` 来自 `{ddl: ...}`；server 侧导出同名扫描入口供索引/API 使用。`org-env` 的 begin/end block 也复用 block command parser；后续插件如果需要自建 inline/block 语法，应优先复用这里的 parser，而不是再写散落正则。
