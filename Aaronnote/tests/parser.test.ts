...
$$`;
  ...
  test("same-line double-dollar math source stays editable text", () => {
    ...
    const p = doc.child(0);
    expect(p.type).toBe(schema.nodes.paragraph);
    expect(p.textContent).toBe("$$ E = mc^2 $$");
  });

  test("org proof block is not parsed as a heading", () => {
    const doc = parse("#+begin proof Proof. test\nline one\nline two\n#+end proof");
    const block = doc.child(0);
    expect(block.type).toBe(schema.nodes.org_env_block);
    expect(block.attrs.kind).toBe("proof");
  ...
  test("org block content supports normal blockquote parsing", () => {
    const doc = parse("#+begin summary\n> quoted\n#+end summary");
    const block = doc.child(0);
    expect(block.type).toBe(schema.nodes.org_env_block);
    expect(block.child(0).type).toBe(schema.nodes.blockquote);
...
describe("parser: inline marks", () => {
  ...
  test("inline math source stays editable text", () => {
    const doc = parse("let $x^2$ be positive");
    const p = doc.child(0);
    expect(p.textContent).toBe("let $x^2$ be positive");
  });

  test("inline double-dollar math source stays editable text", () => {
