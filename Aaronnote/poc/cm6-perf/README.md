...
用真实大文件测 Aaronnote 当前 CM6 编辑路径的性能基线：`createEditor`、live preview、数学/表格/代码块 widgets 和主题样式都会参与测量。
...
```sh
# 启动 POC dev server（从 Aaronnote/ 根目录运行）
...
- 这条路径测的是实际 Aaronnote editor，而不是简化版 CM6 插件。
