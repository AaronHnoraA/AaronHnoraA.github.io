...
`,document.head.appendChild(p);let m=document.createElement(`button`);m.type=`button`,m.dataset.notesTab=`roamlookup`,m.textContent=`Roam lookup`;let h=document.createElement(`div`);h.dataset.notesPanel=`roamlookup`,h.className=`aaronnote-roamlookup-panel`,h.hidden=!0;let g=document.createElement(`header`);g.className=`aaronnote-roamlookup-head`;let _=document.createElement(`div`),v=document.createElement(`strong`);v.textContent=`Roam lookup`;let y=document.createElement(`span`);y.textContent=`Not started`,_.append(v,y);let b=document.createElement(`div`);b.className=`aaronnote-roamlookup-actions`;let x=document.createElement(`button`);x.type=`button`,x.textContent=`Start`;let S=document.createElement(`button`);S.type=`button`,S.textContent=`Close`,b.append(x,S),g.append(_,b);let C=document.createElement(`div`);C.className=`aaronnote-roamlookup-log`;let w=document.createElement(`form`);w.className=`aaronnote-roamlookup-form`;let T=document.createElement(`textarea`);T.placeholder=`Ask the knowledge base`;let E=document.createElement(`button`);E.type=`submit`,E.textContent=`Ask`,w.append(T,E),h.append(g,C,w),a.append(m),o.append(h);function D(e){y.textContent=e,i.setStatus(`RoamLookup: ${e}`)}function O(){window.clearTimeout(u),c&&(u=window.setTimeout(()=>{if(l){O();return}A(`Idle timeout`)},d))}async function k(){if(O(),c)return;D(`Starting`);let e=await n(`/api/roamlookup/start`,{file:i.currentFile()});if(e.disabled)throw Error(e.message||`RoamLookup disabled`);c=e.sessionId||``,d=t(e.idleMs??d,d),D(e.status||`Ready`),O()}async function A(e=`Closed`){window.clearTimeout(u),u=0;let t=c;c=``,l=!1,t&&await n(`/api/roamlookup/close`,{sessionId:t}).catch(()=>({})),D(e)}function j(){s?.hidden&&window.dispatchEvent(new CustomEvent(`aaronnote:command`,{detail:{command:`open-filesystem`}})),document.querySelectorAll(`[data-notes-tab]`).forEach(e=>{e.classList.toggle(`is-active`,e===m)}),document.querySelectorAll(`[data-notes-panel]`).forEach(e=>{e.hidden=e!==h}),k().then(()=>T.focus()).catch(e=>{r(C,`system`,e instanceof Error?e.message:`RoamLookup failed`),D(`Failed`)})}async function M(){let e=T.value.trim();if(!e||l)return;await k(),O(),r(C,`user`,e),T.value=``,l=!0,E.disabled=!0;let t=r(C,`system`,`Running`);D(`Running`);try{let i=await n(`/api/roamlookup/query`,{sessionId:c,query:e});t.remove(),i.sessionId&&(c=i.sessionId),r(C,`assistant`,i.answer||i.message||`No answer`),D(i.status||`Ready`)}catch(e){t.remove(),r(C,`system`,e instanceof Error?e.message:`RoamLookup failed`),D(`Failed`)}finally{l=!1,E.disabled=!1,O(),T.focus()}}let N=e=>{e.preventDefault(),j()};m.addEventListener(`click`,N),f.push(()=>m.removeEventListener(`click`,N));let P=e=>{e.currentTarget!==m&&(h.hidden=!0,m.classList.remove(`is-active`))};return document.querySelectorAll(`[data-notes-tab]`).forEach(e=>{e!==m&&(e.addEventListener(`click`,P),f.push(()=>e.removeEventListener(`click`,P)))}),document.querySelectorAll(`[data-action='notes'], [data-action='agenda']`).forEach(e=>{e.addEventListener(`click`,P),f.push(()=>e.removeEventListener(`click`,P))}),x.addEventListener(`click`,()=>{k().catch(e=>{r(C,`system`,e instanceof Error?e.message:`RoamLookup failed`),D(`Failed`)})}),S.addEventListener(`click`,()=>void A(`Closed`)),w.addEventListener(`submit`,e=>{e.preventDefault(),M()}),T.addEventListener(`input`,O),T.addEventListener(`keydown`,e=>{O(),e.key===`Enter`&&(e.metaKey||e.ctrlKey)&&(e.preventDefault(),M())}),f.push(i.onSettingsChange(n=>{d=t(n.idleMs,e),O()})),f.push(i.onAction(e=>{e===`open`&&j(),e===`close`&&A(`Closed`)})),()=>{A(`Closed`),f.splice(0).forEach(e=>e()),m.remove(),h.remove(),p.remove()}}export{i as setup};