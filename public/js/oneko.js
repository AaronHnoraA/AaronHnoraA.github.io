...
(function oneko() {
  ...
  function init() {
    ...
    // 核心修改：基于当前 JS 文件的路径来寻找 gif，而不是基于 HTML 页面路径
    if (curScript && curScript.src) {
        try {
            // 以当前 JS 文件的 URL 为基准，寻找同目录下的 oneko.gif
            nekoFile = new URL("./oneko.gif", curScript.src).toString();
        } catch (e) {
            console.warn("Oneko: Failed to resolve path relative to script, falling back to default.");
        }
    }

    // 如果用户手动在标签上指定了 data-cat，则优先级最高
