...
#+begin warning direct sum 不是单纯的向量空间直和
在 tensor 语境里，
$ U = U_A ⊕ U_B $
...
满足
$
P(U\_A) = U\_B, quad P(U\_B) = U\_A,
$
$
Q(V\_A) = V\_B, quad Q(V\_B) = V\_A,
$
$
R(W\_A) = W\_B, quad R(W\_B) = W\_A,
$
...
#+begin theorem minimal support spaces 的内在性
对任意
$ T ∈ U ⊗ V ⊗ W $，
上述
...
#+begin proposition Krull–Schmidt 型 refined decomposition 的使用方式
...
#+begin summary 为什么本文主证明不必在一开始就假设 indecomposable
核心 iff
$
A ≅ B ⟺ A plus.o B
$
...
#+begin lemma minimal support spaces 的最小性
对任意
$ T ∈ U ⊗ V ⊗ W $，
第一腿的空间
$
...
#+begin lemma switching automorphism 给出同构
设
$
S = A ⊕ B
$
...
因而
$
(P|_{U_A}, Q|_{V\_A}, R|\_{W\_A})
$
就是从 $A$ 到 $B$ 的同构。
#+end proof

#+begin lemma 同构给出 switching automorphism
若
$
A ≅ B,
$
...
$
P: U\_A ⊕ U\_B → U\_A ⊕ U\_B,
$
$
Q: V\_A ⊕ V\_B → V\_A ⊕ V\_B,
$
$
R: W\_A ⊕ W\_B → W\_A ⊕ W\_B
$
且它们都交换两块。并且
$
(P ⊗ Q ⊗ R)A = B, quad
...
#+begin proposition 对 TA-all 的一跳归约
...
$
P(U\_A)=U\_B,\\ Q(V\_A)=V\_B,\\ R(W\_A)=W\_B
$
以及反向交换。因为整群已经由 oracle 提供，所以这是输出敏感但概念上最干净的版本。
#+end proof

#+begin proposition 对 TA-gen 的 Turing reduction
...
#+begin remark 一个可接受的限制版 positive result
...
#+begin proposition 复杂性估计
记输入尺寸为 $N$。则：

1. minimal support reduction 是线性代数问题，时间多项式于 $N$；
2. 构造 $S = A ⊕ B$ 也是多项式时间；
   3. 若采用 TA-all，后处理复杂度是 $ "Pol"(n) $ 乘以 oracle 输出长度；
3. 若采用 TA-gen 且底域为有限域，则后处理可以借助标准矩阵群与有限置换群的标准算法，在维数与 $log q$ 的多项式时间内完成；
4. 若采用 centroid / idempotent 路线，则在有限域上可调用有限维代数结构算法求 primitive idempotents，通常为随机多项式时间。
#+end proposition

#+begin remark 有限域与无限域对比
...
#+begin summary 本文推荐的 theorem wording
*最稳妥的写法* 是分三层：
...
#+begin warning 对称／交替情形不要与一般三腿 GL 情形混写
...
#+begin theorem 主结构定理草稿
设
$
A ∈ U\_A ⊗ V\_A ⊗ W\_A, quad
B ∈ U\_B ⊗ V\_B ⊗ W\_B
$
