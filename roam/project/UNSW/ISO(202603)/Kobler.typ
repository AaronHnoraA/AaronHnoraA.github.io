...
#summary("Reading Note")[
  This is a note for #zoterolink("zotero://select/items/1_54IJ8DAF")[Köbler, 2020, "ISO(2026-03)"].
  ...
  #note-equation("eq:graph-chain")[
    $ d"GA" <=_p "GI" <=_p "GA," $
  ]

  #note-equation("eq:tensor-chain")[
      $ d"TA" #note-pin("a") <=_p "TI" #note-pin("b") <=_p "cTA." $
  ]

  In @eq:graph-chain, the reduction $d"GA" <=_p "GI"$ can be understood through coloring or encoding operations on graphs. The full graph-side chain has already been established.

  The tensor-side reduction $"TI" <=_p "cTA"$ is already known in our current setting. The unclear part is the remaining tensor analogue, especially whether the graph-side idea behind $d"GA" <=_p "GI"$ has a valid tensor-side counterpart.

  #note-pinit-point-from(("a", "b"))[
    This is the part that is currently unclear.
  ]
]
